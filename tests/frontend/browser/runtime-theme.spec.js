...
test('runtime sidebar color overrides both schemes without rebuilding assets', async ({ page }) => {
    ...
    await expect(page.locator('html')).toHaveAttribute('data-soa-color-scheme', 'light')
    ...
    await expect(page.locator('html')).toHaveAttribute('data-soa-color-scheme', 'dark')
    await expect(page.locator('#sidebar')).toHaveCSS('background-color', 'rgb(18, 52, 86)')

    await page.locator('[data-soa-runtime-properties]').evaluate((style) => style.remove())
