...
- Статус: **реализация TablerTheme и финальный release gate завершены
  2026-09-10**.
...
- Release artifact проверен в чистом Laravel 12 consumer без Node.js; push
...
- [x] Перебазировать plan на текущие name-driven assets и sparse Blade
      ...
      зависимостью.
- [x] Зафиксировать exact upstream release/npm package, source URL, integrity/
      checksum, license и воспроизводимый vendor-update workflow.
- [x] Проверить Bootstrap version/peer tree, Node/browser requirements,
...
- [x] Создать `docs/modernization/tabler-component-inventory.md`: upstream
      component, snapshot, canonical или Tabler Blade owner, states,
      `ThemeCapability` и необходимость vendor JavaScript.
- [x] Отдельно составить Blade difference inventory: какие logical views
      наследуются без изменений и какие действительно требуют sparse override.
- [x] Доказать отсутствие jQuery и определить минимальный Tabler/Bootstrap JS,
      который нельзя заменить существующими package-owned native drivers.
- [x] Зафиксировать аудиторию и single job: backend-разработчики и операторы
      управляют data-heavy CRUD, tables, forms и navigation без знания frontend
      build.
- [x] Зафиксировать отдельный от AdminLTE/Shadcn design brief: palette,
      typography, density, layout и один характерный, функционально оправданный
      motif.
- [x] Снять upstream/reference screens для layout/navigation, sync/async table,
      filters/actions, form, uploads, tree, auth, light/dark и compact viewport.

## 1. Создать прямую TablerTheme

- [x] Добавить `TablerTheme`, напрямую реализующую четыре текущих метода
...
- [x] Закрепить exact style/script order `ThemeRuntimeAssets`, включая deferred
      `shared:modules`, и отсутствие AdminLTE/Shadcn URLs при выборе Tabler.
- [x] Неверный name/class/declaration/capability/manifest должен давать
      диагностическую ошибку без fallback к другой теме.

## 2. Собрать vendor и token boundary

- [x] После checkpoint 0 добавить exact dependencies/lock без jQuery и без
      изменения зависимостей других тем сверх доказанного совместимого reuse.
- [x] Создать самостоятельный `theme:tabler` style entry с одним vendor import
      boundary; не импортировать AdminLTE/Shadcn и не копировать shared CSS.
- [x] Сопоставить Tabler/vendor variables с canonical `--soa-*`; неизвестные
      `--tblr-*` не объявлять public API. Зафиксировать light/dark, typography,
      density, radii, shadows и focus/motion states.
- [x] Проверить `sidebar_background_color → --soa-sidebar-bg` и отсутствие
      theme-owned color writes из JavaScript.
- [x] Оставить Font Awesome в автоматически подключаемом `shared:icons`;
      Tabler icons/fonts/static assets добавлять только после отдельного
      license/size решения из inventory.
- [x] Доказать, что shared/core source ownership не изменился в theme-owned
...
- [x] Собрать production/development profiles с одинаковыми logical ids и
      разными optimization/diagnostics; manifest содержит существующие files,
      MD5-compatible URL versions и SHA-256 checksums.
- [x] `ADMIN_DEV_ASSETS`/`sleeping_owl.dev_assets` выбирает один готовый profile
      целиком без смешивания chunks и без consumer rebuild.
- [x] Оба profiles публикуются одной `php artisan sleepingowl:update` и проходят
      `sleepingowl:update --check`; missing/corrupt/mismatched assets дают точную
      диагностику и восстановительную command.
- [x] Полная functional matrix tables/actions/forms/uploads/tree/lightbox/
      WYSIWYG/Vue islands проходит в обоих profiles.
- [x] Keyboard/focus/ARIA/contrast/reduced-motion/responsive/light-dark browser
      и visual smoke tests проходят на reference screens.
- [x] Asset match/mismatch, locale fallback и отсутствие лишней asset-health
      разметки при match покрыты PHP/browser tests.
- [x] Bundle sizes, dependency/license/static inventory и отсутствие cross-theme
      URLs зафиксированы автоматическим отчётом.
- [x] Документировать выбор `tabler`, additional CSS/JS, application namespace
      overrides и `--soa-*` customization без rebuild; обновить README,
      migration notes, config matrix и CHANGELOG.
- [x] Чистый release artifact устанавливается в Laravel только через Composer/
      PHP/Artisan, без Node.js, и проходит выбор Tabler в production/development
      плюс `sleepingowl:update --check`.
- [x] Финальный gate: `vendor/bin/phpunit`, `npm run check`,
...
| 2026-09-10 | Реализация и production profiles | Зафиксирован `@tabler/core@1.5.1` и MIT notice, добавлены прямой `TablerTheme`, ordered sparse Blade namespace, CSS-only vendor/token/feature boundary, light/dark/responsive/accessibility presentation, общая browser matrix, документация и автоматический bundle report. Production runtime: 10 файлов, 2 108 853 bytes / 469 726 gzip bytes; `theme:tabler`: 691 643 / 80 152 gzip bytes. Узкие PHP, Vitest и Chromium проверки пройдены. | этот commit |
| 2026-09-10 | Финальный release gate | Оба asset profiles собраны и проверены. PHPUnit: 649 tests / 3 398 assertions / 11 skipped; frontend gate: Prettier, ESLint, Stylelint, reachability 345/345 и Vitest 124 files / 550 tests; Playwright: 217/217. Composer archive установлен в чистый Laravel 12 без Node.js; `sleepingowl:update`, `--check`, 18 файлов обоих profiles и runtime тем `adminlte`, `shadcn`, `tabler`, `empty` проверены. | этот commit |
| 2026-09-10 | Vite migration | Laravel Mix/Webpack полностью заменены на Vite 8 и официальный Vue plugin. Сохранены classic-script outputs, оба profile, CSS/JS source maps, license sidecars, MD5/SHA-256 manifest, Font Awesome paths и no-build Composer contract. Tabler production runtime: 10 файлов, 1 925 614 bytes / 441 466 gzip bytes; `theme:tabler`: 708 212 / 81 647 gzip bytes. | этот commit |
