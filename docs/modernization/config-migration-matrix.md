...
| `same key/new implementation` | `template`                                      | Тот же class-string выбирает AdminLTE или custom `ThemeInterface`; legacy `TemplateDefault` получает adapter. Внешний provider может зарегистрировать готовый fragment и явно заменить configured class через `ThemeRegistry`.                         |
