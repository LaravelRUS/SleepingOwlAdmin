...
| `same key/new implementation` | `template`                                      | Новый shape `template.default` + `template.themes` выбирает готовую тему по имени; смена темы использует готовые assets без Node.js/rebuild. Прежний class-string остаётся только runtime fallback, `TemplateDefault` получает adapter, внешний provider может зарегистрировать fragment через `ThemeRegistry`. |
