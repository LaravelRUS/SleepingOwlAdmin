...
Shared consumers use these convenience methods rather than accessing `engineInstance`. Bulk and custom actions serialize the scoped row identifiers with `URLSearchParams`, and auto-update reloads only the tables matched by its configured selector. The legacy `.card` lookup used to associate an action form with its table remains outside core in the AdminLTE-facing compatibility module.
