...
| --- | --- | --- | --- | --- | --- |
| Complete | 1 | `features/table/autoupdate/table-auto-update.js` | Progress line and pause/resume toggle | Historical logical view `default.helper.autoupdate`, backed by the feature host `features.datatables.autoupdate`, supplies a rendered control template | JavaScript clones exactly one control root per matching table, binds pause/resume, timer, registry lifecycle and ProgressBar state. A project override may replace tag, classes, icons and nesting while preserving the documented hooks. |
