...
| --- | --- | --- | --- | --- | --- |
| Complete | 1 | `features/table/autoupdate/table-auto-update.js` | Progress line and pause/resume toggle | Canonical feature view `features.datatables.autoupdate` supplies a rendered control template directly to the base layout | JavaScript clones exactly one control root per matching table, binds pause/resume, timer, registry lifecycle and ProgressBar state. A project override may replace tag, classes, icons and nesting while preserving the documented hooks. |
| Complete | 1 | `features/tree/tree-view.js` | `.soa-tree-toggle` button and `+`/`−` presentation | `default.display.tree_children` renders a toggle for every item that can accept children | `data-tree-toggle` remains the behavior hook. Blade renders both states/text or icons; JavaScript changes only `hidden`, `aria-expanded`, `aria-label` and collapsed state. Leaf toggles stay rendered but hidden so drag-and-drop can reveal them without creating markup. |
| Complete | 2 | `features/table/editing/inline-editor-view.js` | Entire editor root, form, title, input area, actions, error block, buttons and all nine control shapes | `default.column.editable.partials.editor` emits the shared trigger and delegates its Blade `<template>` shell to `partials.editor_template`; genuinely different input shapes remain below `partials.controls`, while checklist keeps one composition view for its display badges | Editable-column PHP classes pass `editorType` and type options directly to the shared host instead of owning one include-only Blade each. Project overrides may replace the shell, one control, classes and nesting while retaining structural hooks. JavaScript clones the template, refreshes the current value, binds submit/cancel/Escape/date lifecycle, and never chooses presentation classes or creates visible elements. |
| Complete | 3 | `features/sidebar/sidebars.js` | `#sidebar-overlay` | Theme layout, currently `default._layout.inner` | The layout renders the established id inside `.wrapper`; `data-widget="pushmenu"` stays unchanged and no `data-sidebar*` marker is added. JavaScript only shows/hides the existing overlay and handles click/Escape/focus/persistence. Static, PHP render and legacy/headless browser contracts protect the boundary. |
...
| --- | --- | --- |
| `[Complete] admin/form/file.vue` | `default.form.element.file` | Blade keeps label/help/error/layout ownership and supplies concrete alert, current-file, upload/download/remove and icon-state classes. Vue owns errors, upload progress, current file state and interactive inner controls through plain `data-file-*` structural hooks. |
