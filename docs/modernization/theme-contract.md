...
`AdminLTETheme` является прямой реализацией `ThemeInterface` и одновременно наследует `TemplateDefault`, пока legacy rendering API остаётся публичным. Она владеет namespace `sleeping_owl::default`, объявляет общие `shared:compatibility`/`shared:vue`/`shared:modules`, единый `theme:adminlte` и восемь capabilities. Обязательный package-owned `shared:icons` добавляется общим runtime. Все встроенные feature presentation adapters компилируются внутрь theme bundle, а не объявляются отдельными runtime entries. Icon classes не преобразуются в PHP: существующие Blade views и пользовательские расширения продолжают задавать нужные Font Awesome classes напрямую.
