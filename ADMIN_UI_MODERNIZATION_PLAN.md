...
- Точка возобновления: `file` island завершён с обычными `data-file-*` hooks; следующим отдельным checkpoint нормализовать ранее введённые в этой ветке `data-soa-*` attributes завершённых `env-editor`, `select` и `related-elements` islands до предметных `data-*`, затем передать Blade-owned classes/options в `image` и `images`, сразу используя простые hooks. Logical view paths, relative `setView()`, fully namespaced `addCustomView()` и настоящий Laravel application-over-package override priority уже закреплены executable contracts. Auto-update, tree toggle, полный inline editor со всеми девятью controls, sidebar overlay и native tooltip popup переведены на Blade-owned markup; tooltip имеет только узкий neutral fallback для минимальных custom themes. После остальных Blade-first contracts поэтапно убрать branch-only префикс `soa` из остальных structural attributes, реализовать notification adapter и переключить `AdminLTETheme` с aggregate на versioned logical bundles, а затем закончить Tailwind presentation без транзитивной загрузки Bootstrap/AdminLTE. Browser entries forms/table/lightbox/tree и profile-aware `shared:compatibility`/`shared:vue` уже готовы вместе с `shared:icons`; новый config default указывает на прямую `AdminLTETheme`, её standalone CSS содержит Bootstrap/AdminLTE, а metadata явно объявляет общие assets и component adapters. Старые опубликованные config с `TemplateDefault` продолжают работать через legacy adapter. DataTables 3 Core работает через `data-table-engine`; отдельно лицензируемый DataTables Editor не используется, опубликованный table runtime не включает библиотеку/runtime dependency jQuery, а server-side processing сохраняет прежний PHP API и wire protocol. Native alert сохранил `data-dismiss="alert"`, как dropdown/tooltip сохранили `data-toggle`, а sidebar — `data-widget`; replacement markers не вводятся.
...
- Базовый commit не содержит `data-soa-*`, поэтому все такие attributes являются branch-only contract и до release поэтапно переименовываются без временных двойных aliases; Sass custom properties `--soa-*` остаются отдельным namespaced style API и под это правило не подпадают.
...
- [ ] Реализовать современную `AdminLTETheme`, инкапсулирующую собственные Bootstrap/AdminLTE assets, markup и component adapters.
  ...
  - [ ] Закрепить Blade-first rendering/override contract до дальнейшего переключения theme runtime.
    ...
    - [ ] Убрать branch-only префикс `soa` из structural `data-*` contracts, сохранив реальные legacy markers без замены.
      - [x] `file`: `data-file-*`.
      - [ ] Завершённые Vue islands: `data-env-*`, `data-select-*`, `data-related-*`.
      - [ ] Оставшиеся Vue islands и общий host contract: `data-image-*`, `data-images-*`, `data-vue-*`.
      - [ ] Native feature contracts: table/inline editor, tree, date, lightbox, tabs и tooltip.
      - [ ] Core/theme state contracts: cloak, loading, selected, visibility и color scheme.
    - [ ] Передавать theme classes/options Vue islands непосредственно из Blade там, где inner precompiled widget нельзя заменить Blade markup без runtime compiler.
      ...
      - [ ] `image`: preview и upload/insert/download/remove controls с обоими icon states.
      - [ ] `images`: gallery, dialog, reorder/upload/insert/remove controls и icons.
