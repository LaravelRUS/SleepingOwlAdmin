...
- Точка возобновления: заменить Bootstrap alert data API нативным delegated controller с сохранением публичного `data-dismiss="alert"`, затем удалить глобальные jQuery/Bootstrap wrappers. Именованные callbacks табличных actions уже получают native DOM values с прежними именами, числом и порядком аргументов; `jquery-form`, пустой Noty wrapper и мёртвый jQuery draft удалены. Новые replacement markers вроде `data-soa-dropdown*` не вводятся: при замене implementation сохраняется существующая разметка, если она не создаёт технический или security blocker.
...
- [ ] Реализовать native alert с сохранением `data-dismiss="alert"`, затем удалить исполняемые jQuery/Bootstrap wrappers.
...
- [ ] Удалить прямой `jquery` и все оставшиеся jQuery-only dependencies из `package.json`.
