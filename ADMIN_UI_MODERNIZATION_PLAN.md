...
- Точка возобновления: заменить native dropdown capability; tabs уже переведены на theme-neutral native driver с сохранением tab state/events, tooltip — на delegated native driver с неизменным публичным marker `data-toggle="tooltip"`, затем остаётся sidebar/push menu/navigation tree. Select2/AJAX/dependent select уже переведены на Vue Multiselect 3, date/datetime/time/daterange — на единый Air Datepicker driver, X-editable — на native inline-editor controller, Nestable tree — на SortableJS driver, а Magnific Popup/lightbox — на GLightbox.
...
- [ ] Theme tooltip/tab/dropdown/sidebar capabilities.
  ...
  - [ ] Native dropdown.
  - [ ] Native sidebar/push menu/navigation tree.
