...
- Точка возобновления: удалить последний исполняемый jQuery boundary — legacy callback adapter табличных actions — затем убрать глобальные jQuery/Bootstrap wrappers, неиспользуемый `jquery-form` и мёртвые комментарии. WYSIWYG wrapper и adapters CKEditor 4/5, TinyMCE и SimpleMDE уже переведены на async-safe `Admin.Components` lifecycle с сохранением `textarea[data-wysiwyg-editor]`, editor names, config и `Admin.WYSIWYG.*`. `File`, `Image` и `Images` работают как Vue 3 islands, а `Files` — через native lifecycle/FormData без изменения прежнего HTML/PHP contract.
