...
- Точка возобновления: заменить единственную live-инициализацию `$(table).DataTable(options)` на изолированный `createDataTables2(element, options)`/`new DataTable(...)` constructor boundary; DataTables 2.3.8, Responsive 3.0.8, официальный Bootstrap 4 adapter и удаление старого renderer уже завершены.
