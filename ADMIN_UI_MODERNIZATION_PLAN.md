...
- Точка возобновления: убрать jQuery-операции из оставшегося Files island. Sidebar/push menu/navigation tree уже переведены на native driver с сохранением публичных markers `data-widget="pushmenu"` и `data-widget="treeview"`; tabs, tooltip и dropdown также работают без Bootstrap/jQuery JS, причём публичные `data-toggle="tooltip"` и `data-toggle="dropdown"` не менялись. Select2/AJAX/dependent select уже переведены на Vue Multiselect 3, date/datetime/time/daterange — на единый Air Datepicker driver, X-editable — на native inline-editor controller, Nestable tree — на SortableJS driver, а Magnific Popup/lightbox — на GLightbox.
