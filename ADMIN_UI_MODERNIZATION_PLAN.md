...
- Точка возобновления: neutral backend extension cookbook и его executable contract закрыты без повторного доступа к reference-проекту. Следующий отдельный пункт — migration branch/checkpoint pilot-проекта после явного разрешения на изменения `D:\domains\laluna.kit`.
