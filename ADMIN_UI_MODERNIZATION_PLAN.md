...
- Точка возобновления: перенести DataTables error handling и custom ordering/search registration с `$.fn.dataTable.ext` на публичный engine API; constructor boundary и актуальные `layout`/`stateSave`/`drawCallback` options уже завершены.
