...
| 2026-09-08 | TailwindTheme / displays, tables и actions | Tailwind namespace получил все 68 `display/*`/`column/*` logical views и shadcn-derived table/action primitives; sync/async DataTables 3, Responsive, filters, pagination, selection, bulk/row actions и inline editor используют theme-owned `soa-*` presentation при сохранённых public hooks/attributes/field names. Tailwind table adapter сведен к canonical `--soa-*`, объявлены `feature:table:theme:tailwind` и `table-presentation`. Оба готовых profiles пересобраны; 96 manifest files и MD5/SHA-256 согласованы, production theme/table CSS — 16 870/14 032 bytes, forbidden framework/runtime scan чист. Узкий gate: PHP 42/396 плюс asset 26/168, frontend 109 source + 221 compiled, Stylelint. Следующая точка — forms/Vue islands/uploads/editors/related. | текущий commit |
| 2026-09-08 | TailwindTheme / forms, Vue islands, uploads и editors | Все 46 `form/*` logical views зеркалированы в Tailwind namespace и дополнены 12 shadcn-derived form primitives. Отдельный `feature:forms:theme:tailwind` adapter оформляет native controls, responsive 12-column grid, actions, uploads/gallery, Vue Multiselect, image dialog, WYSIWYG/Trix и related groups только через canonical `--soa-*`. Классы Vue islands задаются Blade props; сохранены names/attributes/states, CSRF/method/redirect и upload/card hooks, а collapse/maximize работает через native theme driver без AdminLTE/jQuery. Оба профиля пересобраны; 98 файлов прошли MD5/SHA-256, forms CSS production/development — 17 200/21 950 bytes, forbidden runtime/framework/color scan чист. Узкий gate: PHP 76/414, frontend 366, полный Stylelint. Следующая точка — tabs/tree/lightbox adapters и widget/auth views. | текущий commit |
