...
- Точка возобновления: перенести полный inline editor из `features/table/editing/inline-editor-view.js` в переопределяемые Blade `<template>` существующего logical owner `default.column.editable.partials.editor`, включая все девять control types, arbitrary classes/nesting и PHP/browser override contracts. Auto-update close control и tree toggle уже переведены на Blade-owned markup. После остальных Blade-first contracts реализовать notification adapter и переключить `AdminLTETheme` с aggregate на versioned logical bundles, а затем закончить Tailwind presentation без транзитивной загрузки Bootstrap/AdminLTE. Browser entries forms/table/lightbox/tree и profile-aware `shared:compatibility`/`shared:vue` уже готовы вместе с `shared:icons`; новый config default указывает на прямую `AdminLTETheme`, её standalone CSS содержит Bootstrap/AdminLTE, а metadata явно объявляет общие assets и component adapters. Старые опубликованные config с `TemplateDefault` продолжают работать через legacy adapter. DataTables 3 работает через `data-table-engine`; опубликованный table runtime не включает библиотеку/runtime dependency jQuery, а server-side processing сохраняет прежний PHP API и wire protocol. Native alert сохранил `data-dismiss="alert"`, как dropdown/tooltip сохранили `data-toggle`, а sidebar — `data-widget`; replacement markers вроде `data-soa-dropdown*` и `data-soa-alert*` не вводятся.
...
- [ ] Реализовать современную `AdminLTETheme`, инкапсулирующую собственные Bootstrap/AdminLTE assets, markup и component adapters.
  ...
  - [ ] Закрепить Blade-first rendering/override contract до дальнейшего переключения theme runtime.
    ...
    - [ ] Перенести полный inline editor и все девять control types в Blade-rendered templates существующего logical owner с поддержкой произвольных classes и вложенности.
    - [ ] Перенести sidebar overlay в theme layout, сохранив `data-widget="pushmenu"` без replacement markers.
    - [ ] Перенести native tooltip popup в layout-level Blade template, сохранив `data-toggle="tooltip"` и neutral fallback для минимальных custom themes.
