...
- [ ] Реализовать современную `AdminLTETheme`, инкапсулирующую собственные Bootstrap/AdminLTE assets, markup и component adapters.
  ...
  - [ ] Закрепить Blade-first rendering/override contract до дальнейшего переключения theme runtime.
    ...
    - [ ] Передавать theme classes/options Vue islands непосредственно из Blade там, где inner precompiled widget нельзя заменить Blade markup без runtime compiler.
      ...
      - [ ] `related-elements`: wrapper, draggable area, add control и icon.
      - [ ] `env-editor`: cards, table, fields, actions и icons.
      - [ ] `file`: alert и upload/download/remove controls с обоими icon states.
