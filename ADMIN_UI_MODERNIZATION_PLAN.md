...
- Точка возобновления: перевести legacy options (`sDom`, `bStateSave`, `fnDrawCallback`) на актуальные DataTables 2 names; live-инициализация уже использует изолированный `createDataTables2(element, options)`/`new DataTable(...)` constructor boundary без jQuery plugin call.
