...
- Точка возобновления: заменить jQuery-операции в WYSIWYG wrapper и SimpleMDE adapter. `File`, `Image` и `Images` уже работают как Vue 3 islands, а оставшийся legacy `Files` переведён на native lifecycle/FormData с сохранением прежних classes, `data-id` и PHP DSL без новых `data-soa-files*`. Sidebar/push menu/navigation tree уже переведены на native driver с сохранением публичных markers `data-widget="pushmenu"` и `data-widget="treeview"`; tabs, tooltip и dropdown также работают без Bootstrap/jQuery JS, причём публичные `data-toggle="tooltip"` и `data-toggle="dropdown"` не менялись.
...
- [ ] jQuery-операции в WYSIWYG wrappers.
