...
- Точка возобновления: переключить `AdminLTETheme` с compatibility aggregate на versioned logical bundles после подготовки самодостаточных browser entries, затем реализовать полноценную Tailwind presentation без транзитивной загрузки Bootstrap/AdminLTE. Новый config default уже указывает на прямую `AdminLTETheme`, её standalone CSS содержит Bootstrap/AdminLTE, а metadata явно объявляет `shared:icons` и component adapters; старые опубликованные config с `TemplateDefault` продолжают работать через legacy adapter. Этап 6 завершён: first-party runtime не содержит jQuery calls и не создаёт `$`/`jQuery` globals; DataTables 2 использует jQuery только внутри vendor bundle. Native alert сохранил `data-dismiss="alert"`, как dropdown/tooltip сохранили `data-toggle`, а sidebar — `data-widget`; replacement markers вроде `data-soa-dropdown*` и `data-soa-alert*` не вводятся.
