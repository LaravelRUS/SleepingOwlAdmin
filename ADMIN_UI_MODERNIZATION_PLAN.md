...
- Статус: выполняется.
- Текущий этап: **Этап 7 — theme-independent asset health**; этап 8 закрыт, полноценная Tailwind-тема отложена до выбора подходящего готового шаблона.
- Точка возобновления: `AdminLTETheme` работает на versioned logical runtime, Blade-first contract закрыт, DataTables 3 Core скрыт за `data-table-engine`, а реальные `data-dismiss`, `data-toggle` и `data-widget` markers сохранены. jQuery audit/orphan cleanup закрыты; `kodicms/laravel-assets` удалён из Composer manifest, локального lock и установленного tree, при этом строки трёх старых aliases остаются только migration compatibility data и нормализуются в first-party facades. Следующий checkpoint — небольшой request-scoped asset health service и нейтральный status contract; AdminLTE footer реализуется сразу, Tailwind presentation остаётся отложенным вместе с темой.
