...
- Точка возобновления: следующий Tailwind checkpoint — Blade primitives и layout/navigation shell по [`ADMIN_TAILWIND_THEME_PLAN.md`](ADMIN_TAILWIND_THEME_PLAN.md). Прямой `TailwindTheme`, config selection и изолированный logical entry уже готовы. Pilot требует отдельного разрешения на изменения `D:\domains\laluna.kit`; Vite остаётся отдельной post-release задачей.
