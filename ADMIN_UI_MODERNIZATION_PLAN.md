...
- Статус: основной package release gate и встроенная TailwindTheme завершены; post-release вынос постоянных UI-блоков в общий layer ведётся по отдельному checklist.
...
- Точка возобновления: создать общий `shared:ui` и последовательно закрыть [`ADMIN_SHARED_UI_STYLES_PLAN.md`](ADMIN_SHARED_UI_STYLES_PLAN.md). Pilot требует отдельного разрешения на изменения `D:\domains\laluna.kit`; TablerTheme и Vite остаются самостоятельными задачами.
...
- Завершённая встроенная Tailwind-тема ведётся отдельно в [`ADMIN_TAILWIND_THEME_PLAN.md`](ADMIN_TAILWIND_THEME_PLAN.md). Её shadcn/ui snapshot, license/provenance, component inventory и собственный acceptance gate зафиксированы там.
...
- Одинаковые structural/presentation rules постоянных блоков между темами выносятся в обязательный общий layer по [`ADMIN_SHARED_UI_STYLES_PLAN.md`](ADMIN_SHARED_UI_STYLES_PLAN.md); theme plans сохраняют только tokens и реальные отличия.
...
resources/
  js/
    core/{index.js,...}
    shared/features/<id>/{index.js,...}
    themes/<theme-name>/{theme.js,features/<id>/...}
    theme-overrides/<theme-name>/
  css/
    core/{admin-core.scss,_variables.scss,_colors.scss}
    shared/features/<id>/{feature.scss,_variables.scss,_colors.scss}
    themes/<theme-name>/{theme.scss,_variables.scss,_colors.scss,features/<id>/...}
    theme-overrides/<theme-name>/
  views/themes/<theme-name>/
```
...
| 2026-09-06 | Этап 3 / bundle topology | Laravel Mix читает единую декларативную build matrix и выпускает отдельные `core`, `feature:forms`, `feature:table`, `theme:adminlte` и `theme:shadcn` JS/CSS entries, сохраняя пять legacy outputs до переключения resolver. Новые Sass roots задают только cascade layers и не импортируют старый монолит; compiled-contract проверяет content hashes и отсутствие jQuery/Vue/Bootstrap/AdminLTE/DataTables в core. Production build успешен; полный PHP gate: 395 tests, 1456 assertions, 2 прежних TODO-skip; frontend gate: 29 Vitest + 12 Playwright | текущий commit |
...
| 2026-09-08 | TailwindTheme / displays, tables и actions | Tailwind namespace получил все 68 `display/*`/`column/*` logical views и shadcn-derived table/action primitives; sync/async DataTables 3, Responsive, filters, pagination, selection, bulk/row actions и inline editor используют theme-owned `soa-*` presentation при сохранённых public hooks/attributes/field names. Tailwind table adapter сведен к canonical `--soa-*`, объявлены `feature:table:theme:shadcn` и `table-presentation`. Оба готовых profiles пересобраны; 96 manifest files и MD5/SHA-256 согласованы, production theme/table CSS — 16 870/14 032 bytes, forbidden framework/runtime scan чист. Узкий gate: PHP 42/396 плюс asset 26/168, frontend 109 source + 221 compiled, Stylelint. Следующая точка — forms/Vue islands/uploads/editors/related. | текущий commit |
| 2026-09-08 | TailwindTheme / forms, Vue islands, uploads и editors | Все 46 `form/*` logical views зеркалированы в Tailwind namespace и дополнены 12 shadcn-derived form primitives. Отдельный `feature:forms:theme:shadcn` adapter оформляет native controls, responsive 12-column grid, actions, uploads/gallery, Vue Multiselect, image dialog, WYSIWYG/Trix и related groups только через canonical `--soa-*`. Классы Vue islands задаются Blade props; сохранены names/attributes/states, CSRF/method/redirect и upload/card hooks, а collapse/maximize работает через native theme driver без AdminLTE/jQuery. Оба профиля пересобраны; 98 файлов прошли MD5/SHA-256, forms CSS production/development — 17 200/21 950 bytes, forbidden runtime/framework/color scan чист. Узкий gate: PHP 76/414, frontend 366, полный Stylelint. Следующая точка — tabs/tree/lightbox adapters и widget/auth views. | текущий commit |
| 2026-09-08 | TailwindTheme / content adapters и полный namespace | Подключены lightbox/tabs/tree adapters без Bootstrap/AdminLTE; feature colors alias canonical `--soa-*`. Tree notification policy вынесена в отдельный theme entry, слушает публичные `tree:changed`/`tree:failed` и обновляет Blade live region без копии transport/state logic. Добавлены последние dashboard/env-editor/login/tab-badge/CKEditor views; Tailwind namespace теперь покрывает каждый legacy logical path и сохраняет config props, attributes, names, ARIA и hooks. Оба profiles содержат по 50 проверяемых assets. Узкий gate: PHP 20/321, frontend 267, ESLint/Stylelint; следующий checkpoint — Tailwind 4 build/customization/no-build acceptance. | текущий commit |
| 2026-09-08 | TailwindTheme / Tailwind 4 build и no-build distribution | Exact maintainer-only Tailwind/PostCSS 4.3.3 собирают отдельный utility layer без preflight и только из Tailwind namespace; preset использует canonical `--soa-*`. Оба профиля публикуют handwritten и generated CSS под `theme:shadcn`, по 51 asset с MD5/SHA-256; utility CSS production/development — 2 156/2 819 bytes, forbidden framework/runtime/color scan чист. Документированы application utilities, custom properties, Node boundary, config switch и готовая публикация через `sleepingowl:update`. Gate: production build, PHP 14/61, frontend 268, ESLint/Prettier/PHP syntax. Следующая точка — Tailwind acceptance matrix и release gate. | текущий commit |
...
| 2026-09-09 | Config-driven выбор темы | Новый config публикует `template.default` и `template.themes`; resolver проверяет имя и карту, создаёт только выбранную тему и сохраняет runtime fallback для прежнего class-string без legacy-дубликата в package config. Точечные test cases покрывают AdminLTE, `shadcn`, custom/external темы и ошибки выбора; документация и migration matrix синхронизированы. Каталоги/assets в рамках checkpoint не перемещались. Узкий theme gate: 20 tests / 143 assertions. | текущий commit |
