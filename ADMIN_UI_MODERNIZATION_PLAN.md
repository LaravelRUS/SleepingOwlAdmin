...
- Точка возобновления: theme-owned notification adapter дерева закрыт: AdminLTE logical adapter отображает локализованные success/error через собственную SweetAlert policy поверх native `tree:changed`/`tree:failed`, не монтирует дерево, а headless/Tailwind остаются event-only и не получают эту зависимость. Следующий отдельный checkpoint — переключить `AdminLTETheme` с compatibility aggregate на versioned `core + shared + feature + selected theme adapters`, не меняя `TemplateDefault`; после него закончить Tailwind presentation без транзитивной загрузки Bootstrap/AdminLTE. Blade-first contract закрыт полностью: structural/state hooks используют предметные `data-*`, все шесть precompiled Vue islands получают Blade-owned classes/options, а настоящий Laravel application override с произвольными classes и изменённой вложенностью исполняется обоими готовыми asset-профилями без consumer rebuild. Logical view paths, relative `setView()`, fully namespaced `addCustomView()` и Laravel application-over-package override priority закреплены executable contracts. Browser entries forms/table/lightbox/tree и profile-aware `shared:compatibility`/`shared:vue` уже готовы вместе с `shared:icons`; новый config default указывает на прямую `AdminLTETheme`, её standalone CSS содержит Bootstrap/AdminLTE, а metadata явно объявляет общие assets и component adapters. Старые опубликованные config с `TemplateDefault` продолжают работать через legacy adapter. DataTables 3 Core работает через `data-table-engine`; отдельно лицензируемый DataTables Editor не используется, опубликованный table runtime не включает библиотеку/runtime dependency jQuery, а server-side processing сохраняет прежний PHP API и wire protocol. Native alert сохранил `data-dismiss="alert"`, как dropdown/tooltip сохранили `data-toggle`, а sidebar — `data-widget`; replacement markers не вводятся.
