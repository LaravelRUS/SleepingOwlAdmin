...
- Статус: основной package release gate закрыт; TailwindTheme на shadcn/ui получила displays/DataTables/actions presentation, структура третьей TablerTheme зафиксирована, pilot остаётся отложен.
...
- Точка возобновления: следующий Tailwind checkpoint — forms, Vue island props, uploads, editors и related elements по [`ADMIN_TAILWIND_THEME_PLAN.md`](ADMIN_TAILWIND_THEME_PLAN.md). Прямой theme/config selection, canonical tokens, shell и displays/DataTables/actions уже готовы. Pilot требует отдельного разрешения на изменения `D:\domains\laluna.kit`; Vite остаётся отдельной post-release задачей.
