...
- Статус: выполняется.
- Текущий этап: **Этап 8 — удалить jQuery и очистить сборку**; полноценная Tailwind-тема отложена до выбора подходящего готового шаблона.
- Точка возобновления: `AdminLTETheme` переключена с compatibility aggregate на versioned logical runtime `core → shared:icons → shared:compatibility → shared:vue → theme:legacy-adminlte → feature:* + выбранные AdminLTE adapters → shared:modules` в обоих готовых профилях. Legacy `admin-default`, `admin-vue-init` и `admin-modules-load` handles сохранены в рабочих точках расширения; пользовательские modules загружаются после полного runtime и до финального идемпотентного `Admin.Modules.boot()`/component scan. Старый `TemplateDefault` намеренно не менялся и продолжает обслуживать опубликованные config через legacy aggregate. Blade-first contract закрыт: structural/state hooks используют предметные `data-*`, шесть precompiled Vue islands получают Blade-owned classes/options, application overrides исполняются без rebuild. DataTables 3 Core работает через `data-table-engine`, сохраняет server-side wire protocol и не публикует jQuery; DataTables Editor не используется. Реальные `data-dismiss`, `data-toggle` и `data-widget` markers сохранены без replacement aliases. jQuery audit и orphan cleanup закрыты; следующий checkpoint — удалить `kodicms/laravel-assets` из Composer dependency tree, сохранив migration compatibility data.
