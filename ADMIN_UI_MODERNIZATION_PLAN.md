...
- Точка возобновления: выполнить первый Blade-first implementation batch из `docs/modernization/blade-first-ui-inventory.md`: перенести table auto-update close control и tree toggle в переопределяемые Blade templates, затем отдельным checkpoint перенести полный inline editor. После Blade-first contracts реализовать notification adapter и переключить `AdminLTETheme` с aggregate на versioned logical bundles, а затем закончить Tailwind presentation без транзитивной загрузки Bootstrap/AdminLTE. Browser entries forms/table/lightbox/tree и profile-aware `shared:compatibility`/`shared:vue` уже готовы вместе с `shared:icons`; новый config default указывает на прямую `AdminLTETheme`, её standalone CSS содержит Bootstrap/AdminLTE, а metadata явно объявляет общие assets и component adapters. Старые опубликованные config с `TemplateDefault` продолжают работать через legacy adapter. DataTables 3 работает через `data-table-engine`; опубликованный table runtime не включает библиотеку/runtime dependency jQuery, хотя vendor core сохраняет необязательный compatibility code для страниц, которые сами предоставляют jQuery. Native alert сохранил `data-dismiss="alert"`, как dropdown/tooltip сохранили `data-toggle`, а sidebar — `data-widget`; replacement markers вроде `data-soa-dropdown*` и `data-soa-alert*` не вводятся.
