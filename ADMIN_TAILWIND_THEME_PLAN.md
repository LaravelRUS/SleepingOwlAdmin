...
1. **Design brief и tokens.** До компонентов фиксируются плотность data-heavy admin UI, typography, palette, radius, shadows, motion и один отличительный motif. Color literals принадлежат только `_colors.scss`; единый `_tokens.scss` хранит остальные build-time defaults и выпускает runtime `--soa-*` values.
...
├── _colors.scss
├── _tokens.scss            # build-time defaults + canonical --soa-* values
...
- [x] Определить theme-owned `_colors.scss`, `_tokens.scss`, handwritten `theme.scss` и публичные `--soa-*` defaults; generated utility layer не редактировать вручную.
