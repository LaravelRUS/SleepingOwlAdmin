...
- [x] Перенести `soa-*` hooks в общий base markup, сократить Shadcn до четырёх DOM/behavior overrides и архивировать 27 неиспользуемых component prototypes.
...
| 2026-09-09 | Shared semantic Blade markup | Presentation-only class differences сведены в один base contract: legacy Bootstrap/AdminLTE classes сохранены как compatibility aliases, а `soa-*` hooks доступны обеим темам. Из 99 Shadcn overrides оставлены только четыре реальных DOM/behavior boundary (`_layout/inner`, navigation page, display table, login); 27 component prototypes без runtime callers перенесены в `resources/archive/unused-sources`. Оба profiles пересобраны: utility CSS production/development 171/279 bytes; Playwright 141/141. | `0a434cae`, `b2b6f205`, `328c46f0`, `b1f7f516`, `5888bd52`, `43c8c41b`, текущий commit |
